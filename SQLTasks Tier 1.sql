...
SELECT DISTINCT f.name, CONCAT_WS(' ', m.firstname, m.surname) as fullname
FROM Bookings b
JOIN Members m
ON b.memid = m.memid
JOIN Facilities f
...
SELECT f.name, 
	CASE WHEN m.memid > 0 THEN CONCAT_WS(' ', m.firstname, m.surname) 
		 ELSE m.firstname END AS fullname,
...
WHERE DATE(b.starttime) = '2012-09-14' AND 
	(CASE WHEN m.memid > 0 THEN (slots * membercost) > 30
		  ELSE (slots * guestcost) > 30 END) 
ORDER BY cost DESC
